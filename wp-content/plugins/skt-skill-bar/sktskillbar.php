...
function sbar_register_scripts() {
	if ( !is_admin() ) {
		...
		wp_register_script('gage_script', plugins_url('skill_bar/gage/justgage.1.0.1.min.js', __FILE__),'',SB_VER,true);
		wp_enqueue_script('gage_script');

		wp_register_script('gage_raphael_script', plugins_url('skill_bar/gage/raphael.2.1.0.min.js', __FILE__),'',SB_VER,true);
	...
	switch ( $type ){
		...
		case 'circle':
			$wrapCode = '<div class="vertical-page">
				...
				.car li, .skills ul li{display:inline;}
				</style>
